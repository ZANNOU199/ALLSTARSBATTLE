...
export default function BlogNews({ data, setData }: { data: CMSData, setData: React.Dispatch<React.SetStateAction<CMSData>> }) {
  ...
  const handleUpdate = () => {
    setData(prev => ({
      ...prev,
      articles: prev.articles.map(a => a.id === editingId ? { ...a, ...formData } : a)
    }));
    setEditingId(null);
    setFormData({});
  };
    ...
    <div className="space-y-8">
      ...
      {(isAdding || editingId) && (
        <div className="bg-[#111] border border-white/10 p-8 rounded-2xl space-y-6">
          <div className="flex justify-between items-center">
            <h4 className="font-heading text-lg">{isAdding ? 'Nouvel Article' : 'Modifier Article'}</h4>
          ...
          <div className="grid grid-cols-1 gap-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Tag (ex: EVENT, NEWS)</label>
                <input 
                  type="text" 
                  value={formData.tag || ''} 
