...
export default function DashboardOverview({ data }: { data: CMSData }) {
  const stats = [
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="flex gap-4 items-start group">
                <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center shrink-0 group-hover:bg-primary/20 transition-colors">
                  <Clock size={16} className="text-slate-500 group-hover:text-primary" />
                </div>
                <div className="flex-1 border-b border-white/5 pb-4">
                  <div className="flex justify-between items-start mb-1">
                    <p className="text-sm font-bold">Mise à jour du planning</p>
                    <span className="text-[10px] text-slate-500">Il y a 2h</span>
                  </div>
                  <p className="text-xs text-slate-400">Modification de l'activité "Masterclass" du Jour 01.</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-[#111] border border-white/5 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button className="flex flex-col items-center justify-center p-6 rounded-2xl bg-white/5 border border-white/5 hover:border-primary/50 hover:bg-primary/5 transition-all gap-3 group">
